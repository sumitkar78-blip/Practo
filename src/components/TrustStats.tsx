...
export const TrustStats: React.FC = () => {
  const getStatIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'Users':
        return <Users className="w-6 h-6 text-[#1e3a8a]" />;
      case 'MessageSquareHeart':
        return <MessageSquareHeart className="w-6 h-6 text-rose-500" />;
      case 'MapPin':
        return <MapPin className="w-6 h-6 text-sky-600" />;
      default:
        return <ShieldCheck className="w-6 h-6 text-[#1e3a8a]" />;
    }
  };

  return (
    <section id="trust-stats" className="py-16 bg-slate-50/50 border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 mb-12">
          {TRUST_STATS.map((stat) => (
            <div
              key={stat.label}
              className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200 shadow-xs text-center flex flex-col items-center justify-center space-y-2 hover:border-blue-400 transition-colors"
            >
              <div className="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center mb-1">
                {getStatIcon(stat.icon)}
              </div>
              <strong className="text-2xl sm:text-3xl font-black text-[#0f172a] tracking-tight">
                {stat.value}
              </strong>
        ...
        <div className="bg-gradient-to-br from-[#0f172a] via-[#1e3a8a] to-[#0369a1] rounded-3xl p-6 sm:p-10 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-5 space-y-3">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-800/80 text-sky-300 text-xs font-bold border border-slate-700">
            ...
            <div className="lg:col-span-7 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-white/10 backdrop-blur-xs rounded-2xl p-4 border border-white/10 space-y-1.5">
                <div className="w-8 h-8 rounded-lg bg-sky-400/20 text-sky-300 flex items-center justify-center font-bold">
              ...
              <div className="bg-white/10 backdrop-blur-xs rounded-2xl p-4 border border-white/10 space-y-1.5">
                <div className="w-8 h-8 rounded-lg bg-blue-400/20 text-blue-300 flex items-center justify-center font-bold">
